...
(function () {
  ...
  function createPreview(canvasEl) {
    ...
    // where the track is already reachable by URL and doesn't need to be
    // fetched into bytes first. This lets the browser's native <video>
    // loading handle any cross-origin/file access itself, the same
    // mechanism every uploaded video already relies on, instead of going
    // through fetch()/XHR — whose scheme restrictions can be stricter than a
    // <video src> load in some browser security configurations.
